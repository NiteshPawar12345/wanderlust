...
function Login({ setUser, showNotification }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showNotification('Welcome back to Wanderlust!', 'success');
        navigate('/');
      }
    } catch (err) {
      console.error(err);
      showNotification(err.response?.data?.error || 'Login failed. Please check your credentials.', 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="relative w-full min-h-[70vh] flex items-center justify-center py-12 px-4 sm:px-6 transition-colors duration-300">
      ...
      <div className="absolute top-10 left-1/4 w-72 h-72 bg-rose-200/50 dark:bg-rose-950/10 rounded-full blur-[100px] pointer-events-none -z-10 animate-pulse"></div>
      <div className="absolute bottom-10 right-1/4 w-80 h-80 bg-pink-200/40 dark:bg-pink-950/5 rounded-full blur-[120px] pointer-events-none -z-10"></div>
      ...
      <div className="backdrop-blur-xl bg-white/75 dark:bg-slate-900/60 border border-white/40 dark:border-slate-800/80 p-10 rounded-[32px] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.04)] dark:shadow-[0_25px_50px_rgba(0,0,0,0.2)] w-full max-w-md space-y-8 flex flex-col items-center hover:shadow-[0_30px_60px_-15px_rgba(254,66,77,0.06)] dark:hover:shadow-rose-955/10 transition-all duration-300">
        ...
        <div className="flex flex-col items-center text-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-955 dark:text-white tracking-tight">Welcome Back</h2>
            <p className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase tracking-wider mt-1.5">Log in to Wanderlust</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-5">
          ...
          >
            {submitting ? 'Verifying details...' : 'Log in'}
          </button>
        ...
        <div className="text-sm font-semibold text-slate-500 dark:text-slate-400">
          ...
          <Link to="/signup" className="text-brand hover:text-brand-dark hover:underline font-bold">
            Sign up
          </Link>
        </div>
